    ...
    fun test() {
        try {
            ...
            Controller.init(licenseKeyCredentials) { error ->
                ...
                val SD_CARD_PATH = Environment.getExternalStorageDirectory().toString() + "/SharpnessTestImages/"
                val files = getListFiles(File(SD_CARD_PATH))

                for (i in files.indices) {
                    val imageName = files.get(i).getName()
                    if (!imageName.contains("sharpness_")) {
                        continue
                    }
                    ...
                    val sharpness : Double = image.sharpnessGrade.toDouble()
                    val actualSharpness = java.lang.Float.parseFloat(imageName.split("_".toRegex()).dropLastWhile({ it.isEmpty() }).toTypedArray()[2].replace(".jpg", ""))
                    assertEquals(sharpness, actualSharpness.toDouble(),0.001)
                }
                sdk?.cleanup()
                signal.countDown()

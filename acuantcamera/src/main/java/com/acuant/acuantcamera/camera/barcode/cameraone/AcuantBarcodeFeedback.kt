...
data class AcuantBarcodeFeedback(val feedback: BarcodeFeedback, val point: Array<Point>?, val frameSize: Size?, val barcode: String? = null, val detectTime: Long = -1) {
    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (javaClass != other?.javaClass) return false

        other as AcuantBarcodeFeedback
